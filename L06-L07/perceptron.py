...
class simple_perceptron:
    ...
    def __init__(self, epochs=100, learning_rate=0.1, activation='tanh', use_bias=True, use_momentum=True):
        '''
        Constructor.
        Parameters:
        ...
        self.use_bias = use_bias  # Add use_bias to constructor
        self.use_momentum = use_momentum  # Add use_momentum to constructor
        self.alpha = 0.9  # Add alpha to constructor
        self.momentum = 0  # Add momentum to constructor
        random_seed = 100
    ...
    def read_input_data(self, filename, normalize=True):
        ...
        if self.use_bias:
            # Add bias input to each input vector
    ...
    def initialize_weights(self):
        ...
        self.weights = np.random.random(self.Nin + int(self.use_bias))  # Add bias weight if int(use_bias=True)=1 else int(use_bias=False)=0
    ...
    def train(self, Xtrain, Ytrain, Xvalid, Yvalid):
        ...
        for epoch in range(self.epochs):
            ...
            for i in range(len(Xtrain)):
                ...
                if self.use_bias:
                    sumWeighted += self.weights[-1]  # Bias weight
                ...
                for j in range(self.Nin):
                    if self.use_momentum:
                        self.momentum = \
                            self.alpha * self.momentum + self.learning_rate * self.fp(sumWeighted) * (Ytrain[i] - Yout) * Xtrain[i][j]  # Update momentum
                        self.weights[j] += self.momentum
                    else:
                        self.weights[j] += \
                    ...
                    if self.use_bias:
                        if self.use_momentum:
                            self.momentum = \
                                self.alpha * self.momentum + self.learning_rate * self.fp(sumWeighted) * (Ytrain[i] - Yout)  # Update momentum to reflect bias weight
                            self.weights[-1] += self.momentum
                        else:
                            self.weights[-1] += \
                                self.learning_rate * self.fp(sumWeighted) * (Ytrain[i] - Yout)  # Update bias weight
    ...
    def save_model(self, filename):
        ...
        data = {'nin':self.Nin,
                ...
                'use_bias': self.use_bias,  # Save use_bias
                'use_momentum': self.use_momentum,  # Save use_momentum
                'alpha': self.alpha,  # Save alpha
                'momentum': self.momentum,  # Save momentum
    ...
    def load_model(self, filename):
        ...
        try:
            ...
            self.use_bias = data['use_bias']  # Load use_bias
            self.use_momentum = data['use_momentum']  # Load use_momentum
            self.alpha = data['alpha']  # Load alpha
            self.momentum = data['momentum']  # Load momentum
